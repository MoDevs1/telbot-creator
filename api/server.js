...
// Bot Routes
app.post("/api/create-bot", ensureAuthenticated, async (req, res) => {
  try {
...
app.get("/api/my-bots", ensureAuthenticated, async (req, res) => {
  try {
    const encryptedEmail = req.user.email
...
app.get("/bot-creator.html", ensureAuthenticatedPage, (req, res) => {
  res.sendFile(path.join(__dirname, "../public", "bot-creator.html"))
})

app.get("/profile.html", ensureAuthenticatedPage, (req, res) => {
