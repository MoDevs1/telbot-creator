...
function validateSignupForm(formData) {
  ...
  if (phone && !isValidInternationalPhone(phone)) {
    showError("phoneError", "رقم الهاتف غير صحيح. يجب أن يبدأ برمز البلد مثل +966 أو +20")
    isValid = false
  }
  ...
  switch (fieldName) {
    ...
    case "phone":
      if (value && !isValidInternationalPhone(value)) {
        showError(field.id + "Error", "رقم الهاتف غير صحيح. يجب أن يبدأ برمز البلد")
      }
      break
  }
}
...
function isValidInternationalPhone(phone) {
  ...
  // Check if it has at least 7 digits after country code (minimum international standard)
  // and maximum 15 digits total (ITU-T E.164 standard)
